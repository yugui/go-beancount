...
// TestQueryDirectiveTablesByName exercises the directive-filter tables
// end-to-end through the compiler, confirming bare table names in FROM resolve
// and that the upstream-renamed columns (events.type/description,
// documents.filename, commodities.currency) are selectable.
func TestQueryDirectiveTablesByName(t *testing.T) {
	l := &ast.Ledger{}
	l.InsertAll([]ast.Directive{
