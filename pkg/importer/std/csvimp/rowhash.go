...
// normalisation is applied. The instance name is included so the same
// raw row interpreted under two different instances hashes differently.
func rowHash(name string, fields []string) string {
	h := sha256.New()
	h.Write([]byte(name))
