...
func TestErrorAsValidationError(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: validation.Error{
				Code:    validation.CodeMultipleAutoPostings,
				Span:    span,
				Message: "too many auto postings",
			},
		},
		{
			...
			want: validation.Error{
				Code:    validation.CodeInternalError,
				Span:    span,
				Message: "no lot",
			},
		},
		...
		},
		{
			...
			want: validation.Error{
				Code:    "invalid-booking-method",
				Span:    span,
				Message: "Assets:Cash: unknown booking keyword",
			},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Errorf("FromInventoryError mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
