...
// posting count via resolveBookedCosts; the caller's pre-Walk snapshot
// remains untouched.
//
...
// mkLotCost constructs an [ast.Cost] value (the lot-identity record
// stored on a [ReductionStep]). Tests use it to assemble the synthetic
// booked output resolveBookedCosts consumes without going through
// bookOne / Inventory.Reduce.
func mkLotCost(t *testing.T, number, currency string, date time.Time, label string) ast.Cost {
	t.Helper()
	return ast.Cost{
		Number:   decimalVal(t, number),
		Currency: currency,
		Date:     date,
		Label:    label,
	}
...
// TestResolveBookedCosts_SingleLotReduction exercises the pass-through
// branch: a booked slice with no multi-lot reductions still triggers
// the rebuild of txn.Postings (resolveBookedCosts has no fast path),
// but each posting flows through unchanged in shape — the *ast.Cost
// for the single matched lot is installed, BookedPosting.Source is
// remapped to the new slice, and the order is preserved.
func TestResolveBookedCosts_SingleLotReduction(t *testing.T) {
	lotDate := time.Date(2025, 1, 10, 0, 0, 0, 0, time.UTC)
	txn := &ast.Transaction{
		...
		Postings: []ast.Posting{
			{
				...
				Amount:  mkAmountPtr(t, "-5", "STOCK"),
				Cost:    &ast.CostSpec{}, // parse-tier; will be installed
			},
			{Account: "Assets:Cash", Amount: mkAmountPtr(t, "30.00", "USD")},
		},
	}
	step := ReductionStep{Lot: mkLotCost(t, "6.00", "USD", lotDate, "")}
	step.Units.Set(&decimal5)
	booked := []BookedPosting{
		{
			Source:     &txn.Postings[0],
			Account:    "Assets:Stock",
			Units:      ast.Amount{Number: decimalVal(t, "-5"), Currency: "STOCK"},
			Reductions: []ReductionStep{step},
		},
	}

	newBooked, newUnknowns := resolveBookedCosts(txn, booked, nil)

	if got, want := len(txn.Postings), 2; got != want {
		t.Fatalf("resolveBookedCosts: len(txn.Postings) = %d, want %d", got, want)
	}
	if len(newBooked) != 1 || newBooked[0].Source != &txn.Postings[0] {
	...
	if len(newUnknowns) != 0 {
		t.Errorf("resolveBookedCosts: len(newUnknowns) = %d, want 0", len(newUnknowns))
	}
	cost, ok := txn.Postings[0].Cost.(*ast.Cost)
	if !ok || cost == nil {
		t.Fatalf("resolveBookedCosts: Postings[0].Cost type = %T, want *ast.Cost", txn.Postings[0].Cost)
	}
	wantCost := &ast.Cost{
		Number:   decimalVal(t, "6.00"),
		Currency: "USD",
		Date:     lotDate,
	}
	if diff := cmp.Diff(wantCost, cost, astCmpOpts...); diff != "" {
		t.Errorf("resolveBookedCosts: installed Cost mismatch (-want +got):\n%s", diff)
	}
	if !cost.IsBooked() {
		t.Errorf("resolveBookedCosts: installed Cost reports IsBooked()=false; second run would re-clone unnecessarily")
	}
}

// TestResolveBookedCosts_NegativeMultiLot exercises the multi-lot
// expansion branch on a negative-parent reduction matching two lots.
// The parent posting is replaced by two children whose Amount.Number
// magnitudes match step.Units, whose sign matches the parent's, whose
// Cost is *ast.Cost rendering the matched lot's identity, and whose
// BookedPosting.Source points into the rebuilt txn.Postings.
func TestResolveBookedCosts_NegativeMultiLot(t *testing.T) {
	lot1Date := time.Date(2025, 1, 10, 0, 0, 0, 0, time.UTC)
	lot2Date := time.Date(2025, 2, 15, 0, 0, 0, 0, time.UTC)
	parent := ast.Posting{
		Account: "Assets:Stock",
		Amount:  mkAmountPtr(t, "-15", "STOCK"),
		Cost:    &ast.CostSpec{},
	}
	cashLeg := ast.Posting{Account: "Assets:Cash", Amount: mkAmountPtr(t, "90.00", "USD")}
	txn := &ast.Transaction{
		Date:     time.Date(2025, 3, 1, 0, 0, 0, 0, time.UTC),
		Flag:     '*',
		Postings: []ast.Posting{parent, cashLeg},
	}
	step0 := ReductionStep{Lot: mkLotCost(t, "5.00", "USD", lot1Date, "")}
	step0.Units.Set(&decimal10)
	step1 := ReductionStep{Lot: mkLotCost(t, "6.00", "USD", lot2Date, "")}
	step1.Units.Set(&decimal5)
	booked := []BookedPosting{
		{
			Source:     &txn.Postings[0],
			Account:    "Assets:Stock",
			Units:      ast.Amount{Number: decimalVal(t, "-15"), Currency: "STOCK"},
			Reductions: []ReductionStep{step0, step1},
		},
		{
			Source:  &txn.Postings[1],
			Account: "Assets:Cash",
			Units:   ast.Amount{Number: decimalVal(t, "90.00"), Currency: "USD"},
		},
	}

	newBooked, newUnknowns := resolveBookedCosts(txn, booked, nil)

	if got, want := len(txn.Postings), 3; got != want {
		t.Fatalf("resolveBookedCosts: len(txn.Postings) = %d, want %d (multi-lot reduction expanded into 2 children + 1 unchanged leg)", got, want)
	}
	if got, want := len(newBooked), 3; got != want {
		t.Fatalf("resolveBookedCosts: len(newBooked) = %d, want %d", got, want)
	}
	if len(newUnknowns) != 0 {
		t.Errorf("resolveBookedCosts: len(newUnknowns) = %d, want 0", len(newUnknowns))
	}
	...
	wantChildren := []wantChild{
		{signed: "-10", number: "5.00", date: lot1Date},
		{signed: "-5", number: "6.00", date: lot2Date},
	}
	for i, want := range wantChildren {
		child := &txn.Postings[i]
		// Source must alias into the rebuilt slice (load-bearing
		// for the visitor contract).
		if newBooked[i].Source != child {
			t.Errorf("resolveBookedCosts: newBooked[%d].Source = %p, want &txn.Postings[%d] = %p", i, newBooked[i].Source, i, child)
		}
		wantAmount := &ast.Amount{Number: decimalVal(t, want.signed), Currency: "STOCK"}
		if diff := cmp.Diff(wantAmount, child.Amount, astCmpOpts...); diff != "" {
			t.Errorf("resolveBookedCosts: child[%d].Amount mismatch (-want +got):\n%s", i, diff)
		}
		cost, ok := child.Cost.(*ast.Cost)
		if !ok || cost == nil {
			t.Fatalf("resolveBookedCosts: child[%d].Cost type = %T, want *ast.Cost", i, child.Cost)
		}
		wantCost := &ast.Cost{
			Number:   decimalVal(t, want.number),
			Currency: "USD",
			Date:     want.date,
		}
		if diff := cmp.Diff(wantCost, cost, astCmpOpts...); diff != "" {
			t.Errorf("resolveBookedCosts: child[%d].Cost mismatch (-want +got):\n%s", i, diff)
		}
		// Reductions on the new BookedPosting: single step echoing
		// the parent's step contribution.
		if len(newBooked[i].Reductions) != 1 {
			t.Fatalf("resolveBookedCosts: newBooked[%d].Reductions: got %d steps, want 1", i, len(newBooked[i].Reductions))
		}
	}

	// Pass-through leg lands at index 2 with Source aliasing into
	// the rebuilt slice.
	if newBooked[2].Source != &txn.Postings[2] {
		t.Errorf("resolveBookedCosts: newBooked[2].Source = %p, want &txn.Postings[2] = %p", newBooked[2].Source, &txn.Postings[2])
	}
	if newBooked[2].Account != "Assets:Cash" {
		t.Errorf("resolveBookedCosts: newBooked[2].Account = %q, want Assets:Cash", newBooked[2].Account)
	}
}

// TestResolveBookedCosts_PositiveMultiLot pins the sign-bit copy on a
// positive-parent reduction. While unusual in normal ledgers, a
// short position reduced by a positive-units posting under
// BookingNone produces this shape and the expansion must propagate
// the absent Negative bit verbatim.
func TestResolveBookedCosts_PositiveMultiLot(t *testing.T) {
	lotDate := time.Date(2025, 1, 10, 0, 0, 0, 0, time.UTC)
	parent := ast.Posting{
		Account: "Assets:Short",
		Amount:  mkAmountPtr(t, "12", "STOCK"),
		Cost:    &ast.CostSpec{},
	}
	txn := &ast.Transaction{
		Date:     time.Date(2025, 3, 1, 0, 0, 0, 0, time.UTC),
		Flag:     '*',
		Postings: []ast.Posting{parent},
	}
	step0 := ReductionStep{Lot: mkLotCost(t, "-50", "USD", lotDate, "a")}
	step0.Units.Set(&decimal7)
	step1 := ReductionStep{Lot: mkLotCost(t, "-55", "USD", lotDate, "b")}
	step1.Units.Set(&decimal5)
	booked := []BookedPosting{
		{
			Source:     &txn.Postings[0],
			Account:    "Assets:Short",
			Units:      ast.Amount{Number: decimalVal(t, "12"), Currency: "STOCK"},
			Reductions: []ReductionStep{step0, step1},
		},
	}
	...
	wantSigned := []string{"7", "5"}
	for i, want := range wantSigned {
		wantAmount := &ast.Amount{Number: decimalVal(t, want), Currency: "STOCK"}
		if diff := cmp.Diff(wantAmount, txn.Postings[i].Amount, astCmpOpts...); diff != "" {
			t.Errorf("resolveBookedCosts: child[%d].Amount mismatch — positive parent must not flip sign (-want +got):\n%s", i, diff)
		}
	}
}

// TestResolveBookedCosts_RemapUnknowns pins the unknown remap: a
// pass-through unknown posting that co-exists with a multi-lot
// reduction must be reported back with a pointer into the rebuilt
// txn.Postings at the right index. Pass 3 (residual solve) consumes
// this pointer.
func TestResolveBookedCosts_RemapUnknowns(t *testing.T) {
	lot1Date := time.Date(2025, 1, 10, 0, 0, 0, 0, time.UTC)
	lot2Date := time.Date(2025, 2, 15, 0, 0, 0, 0, time.UTC)
	parent := ast.Posting{
		Account: "Assets:Stock",
	...
	autoLeg := ast.Posting{Account: "Income:Capital"} // Amount == nil; auto-posting unknown
	txn := &ast.Transaction{
		Date:     time.Date(2025, 3, 1, 0, 0, 0, 0, time.UTC),
		Flag:     '*',
		Postings: []ast.Posting{parent, autoLeg},
	}
	step0 := ReductionStep{Lot: mkLotCost(t, "5.00", "USD", lot1Date, "")}
	step0.Units.Set(&decimal10)
	step1 := ReductionStep{Lot: mkLotCost(t, "6.00", "USD", lot2Date, "")}
	...
	unknowns := []*ast.Posting{&txn.Postings[1]}

	_, newUnknowns := resolveBookedCosts(txn, booked, unknowns)

	if len(newUnknowns) != 1 {
		t.Fatalf("resolveBookedCosts: len(newUnknowns) = %d, want 1", len(newUnknowns))
	}
	// The auto leg followed the two expansion children, so it now
	// lives at index 2 in the rebuilt slice.
	if newUnknowns[0] != &txn.Postings[2] {
		t.Errorf("resolveBookedCosts: newUnknowns[0] = %p, want &txn.Postings[2] = %p (unknown must remap to rebuilt slice)",
			newUnknowns[0], &txn.Postings[2])
	}
	if newUnknowns[0].Account != "Income:Capital" {
		t.Errorf("resolveBookedCosts: newUnknowns[0].Account = %q, want Income:Capital", newUnknowns[0].Account)
	}
}

// TestResolveBookedCosts_TwoIndependentMultiLots covers a transaction
// carrying two separate multi-lot reductions on different accounts.
// The rebuild must interleave each expansion at the position of its
// parent, leaving Source pointers correct for both the earlier and
// later booked entries — a regression here would only surface on
// transactions that consolidate multiple per-account sales into one
// directive.
func TestResolveBookedCosts_TwoIndependentMultiLots(t *testing.T) {
	lotADate := time.Date(2025, 1, 10, 0, 0, 0, 0, time.UTC)
	lotBDate := time.Date(2025, 2, 15, 0, 0, 0, 0, time.UTC)
	stockA := ast.Posting{
		Account: "Assets:StockA",
		Amount:  mkAmountPtr(t, "-12", "AAA"),
		Cost:    &ast.CostSpec{},
	}
	stockB := ast.Posting{
		Account: "Assets:StockB",
		Amount:  mkAmountPtr(t, "-7", "BBB"),
		Cost:    &ast.CostSpec{},
	}
	cash := ast.Posting{Account: "Assets:Cash", Amount: mkAmountPtr(t, "200.00", "USD")}
	txn := &ast.Transaction{
		Date:     time.Date(2025, 3, 1, 0, 0, 0, 0, time.UTC),
		Flag:     '*',
		Postings: []ast.Posting{stockA, stockB, cash},
	}
	stepA0 := ReductionStep{Lot: mkLotCost(t, "5.00", "USD", lotADate, "")}
	stepA0.Units.Set(&decimal10)
	stepA1 := ReductionStep{Lot: mkLotCost(t, "6.00", "USD", lotBDate, "")}
	stepA1.Units.Set(&decimal2)
	stepB0 := ReductionStep{Lot: mkLotCost(t, "8.00", "EUR", lotADate, "")}
	stepB0.Units.Set(&decimal5)
	stepB1 := ReductionStep{Lot: mkLotCost(t, "9.00", "EUR", lotBDate, "")}
	stepB1.Units.Set(&decimal2)
	booked := []BookedPosting{
		{
			Source:     &txn.Postings[0],
			Account:    "Assets:StockA",
			Units:      ast.Amount{Number: decimalVal(t, "-12"), Currency: "AAA"},
			Reductions: []ReductionStep{stepA0, stepA1},
		},
		{
			Source:     &txn.Postings[1],
			Account:    "Assets:StockB",
			Units:      ast.Amount{Number: decimalVal(t, "-7"), Currency: "BBB"},
			Reductions: []ReductionStep{stepB0, stepB1},
		},
		{
			Source:  &txn.Postings[2],
			Account: "Assets:Cash",
			Units:   ast.Amount{Number: decimalVal(t, "200.00"), Currency: "USD"},
		},
	}

	newBooked, _ := resolveBookedCosts(txn, booked, nil)

	// Final layout: A0, A1, B0, B1, Cash. Five postings; five booked
	// entries with Source pointers into the new slice.
	if got, want := len(txn.Postings), 5; got != want {
		t.Fatalf("resolveBookedCosts: len(txn.Postings) = %d, want %d", got, want)
	}
	if got, want := len(newBooked), 5; got != want {
		t.Fatalf("resolveBookedCosts: len(newBooked) = %d, want %d", got, want)
	}
	for i := range newBooked {
		if newBooked[i].Source != &txn.Postings[i] {
			t.Errorf("resolveBookedCosts: newBooked[%d].Source = %p, want &txn.Postings[%d] = %p",
				i, newBooked[i].Source, i, &txn.Postings[i])
		}
	}
	wantAccounts := []ast.Account{"Assets:StockA", "Assets:StockA", "Assets:StockB", "Assets:StockB", "Assets:Cash"}
	for i, want := range wantAccounts {
		if txn.Postings[i].Account != want {
			t.Errorf("resolveBookedCosts: txn.Postings[%d].Account = %q, want %q", i, txn.Postings[i].Account, want)
		}
	}
	wantAmounts := []*ast.Amount{
		{Number: decimalVal(t, "-10"), Currency: "AAA"},
		{Number: decimalVal(t, "-2"), Currency: "AAA"},
		{Number: decimalVal(t, "-5"), Currency: "BBB"},
		{Number: decimalVal(t, "-2"), Currency: "BBB"},
		{Number: decimalVal(t, "200.00"), Currency: "USD"},
	}
	for i, want := range wantAmounts {
		if diff := cmp.Diff(want, txn.Postings[i].Amount, astCmpOpts...); diff != "" {
			t.Errorf("resolveBookedCosts: txn.Postings[%d].Amount mismatch (-want +got):\n%s", i, diff)
		}
	}
}

// decimalN are shared positive-magnitude constants for the
// resolveBookedCosts tests. Constructing them at package scope keeps
// the per-test setup focused on the expansion contract rather than
// decimal arithmetic.
var (
	decimal2  = mustParseDecimal("2")
	decimal5  = mustParseDecimal("5")
	decimal7  = mustParseDecimal("7")
	decimal10 = mustParseDecimal("10")
)
