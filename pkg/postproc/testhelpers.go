...
// (most notably pkg/postproc/goplug). A blank line separates this
...
// as pkg/postproc/goplug.
//
// The registry lives inside package postproc and we need a handle
// reachable from pkg/postproc/goplug's test binary. A non-_test.go
...
// given. Consumers of :postproc without gotags=["testhelpers"] do
// not see ResetForTest; a direct reference without the tag fails to
// compile.
//
// Bazel additionally enforces this via a separate
// :postproc_testhelpers target marked testonly = True, so
// non-testonly targets cannot depend on the tagged variant of the
// library.

package postproc

import "github.com/yugui/go-beancount/pkg/postproc/api"
...
// pkg/postproc/goplug) that need to start from a clean registry.
//
