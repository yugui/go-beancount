...
##### 緊張1の決着: resolved unknown の `BookedPosting` は `bookedDesc` 経由で currency 付き参照する

Pass 2 が unknown を解決して `bookOne` を成功させ `BookedPosting` を生成したとき、`pr.booked` に append するのに加えて **`pr.bookedDesc` にも `groupRef{currency: residual.Currency, postingAt: <その unknown の unknownDesc エントリの postingAt>}` を append する**。これにより `bookedDesc` が Pass 1 + Pass 2 の全 booked エントリを currency 付きで uniform にカバーし、Step 5 の inventory 逆操作・Source 再束縛・`txn.Postings` rebuild が `bookedDesc` 一本走査で書ける。`unknownDesc[k].currency` も併せて `residual.Currency` に設定する（rebuild 用）。生存した resolved unknown は `bookedDesc` と `unknownDesc` の両方に同一 postingAt で登場するが、Step 5 の rebuild は postingAt 重複を許容する設計とする。

却下: 「`bookedDesc` に足さず `unknownDesc` を Step 5 が cross-reference」案 → Step 5 が二重走査になり「`unknownDesc` のどれが bookOne 済みか」を表す `groupRef` 追加フラグが要る（`currency != ""` だけでは D6 と区別不能）。「`BookedPosting` に weight currency フィールド追加」案 → visitor 公開型を汚す。

##### 緊張2の決着: Step 4 は `finalize` を触らない

`finalize` の呼び出し位置（Pass 2 の前）・シグネチャ・責務は Step 4 では **一切変更しない**。根拠: 現状 Pass 2 の resolved unknown の `BookedPosting.Source` は `finalize` 経由ではなく Pass 2 が自前で `unknownP` を直接設定しており（`unknownP` は `finalize` が返した安定 offset）、これは正しく動く。Step 4 の `bookedDesc` への append は `finalize` 実行後に起き、Step 5 用のメタ情報としてのみ機能する。`finalize` の責務分割・呼び出し位置再構成は Step 5（Pass 2 完了後の drop 適用）の責務。`finalize` 後に `bookedDesc` が `booked` より長くなる非対称は Step 5 が扱う既知事項。

##### Pass 2 配線後の経路構造（ロック対象）

`solveResidual` 呼び出し後、`k` を処理中 unknown の `unknownDesc` インデックスとして:

1. **複数 unknown（`len(unknowns) > 1`）**: `flagAmbiguousUnknowns(unknowns)`。`unknownDesc[*].currency` は `""` のまま。`bookOne` 走らない。**現状と同一。**
2. **`solveResidual` 失敗（`ok == false`）**: 現状どおり `break`。`unknownDesc[k].currency` は `""` のまま。**現状と同一。**
3. **`solveResidual` 成功、`residual.Currency` が `dropped` に存在（D6）**: `unknownDesc[k].currency = residual.Currency` を設定。`bookOne` も `prepareForEdit` も呼ばない。`unknownP.Amount`/`Cost` への residual 反映も行わない（消える posting を touch しない）。`markForDrop` 追加呼び出し不要（既に dropped）。`BookedPosting`・`bookedDesc` append 無し。
4. **`solveResidual` 成功、`residual.Currency` が `dropped` に無い（生存 group / 新規 currency）**:
   - 現状どおり inferred 判定 → `unknownP.Amount`/`Cost` を residual から設定（`resolveCostFromResidual` 失敗は現状どおり `r.errs` append + `break`）
   - `inv := trace.prepareForEdit(unknownP.Account)` → `bookOne(...)`（現状どおり）
   - `bookOne` 成功 かつ `len(steps) <= 1`: 現状どおり `booked = append(booked, BookedPosting{Source: unknownP, ...})`。**追加**: `pr.bookedDesc` に `groupRef{currency: residual.Currency, postingAt: unknownDesc[k].postingAt}` を append、`unknownDesc[k].currency = residual.Currency` を設定。
   - `bookOne` 成功 だが `len(steps) > 1`: 現状どおり `CodeInternalError` + `break`。`bookedDesc` append も `unknownDesc[k].currency` 設定も行わない（`""` のまま）。
   - `bookOne` 失敗: 現状の `r.errs` append に加え **`pr.markForDrop(residual.Currency)`**、`unknownDesc[k].currency = residual.Currency` も設定。`BookedPosting`・`bookedDesc` append 無し。

##### 更新規約（ロック対象）

- `unknownDesc[k].currency` は **`solveResidual` 成功時のみ** `residual.Currency` を書き込む。失敗・ambiguous・`len(steps)>1` の各経路では `""` 据え置き。
- `markForDrop(residual.Currency)` は **経路4の `bookOne` 失敗時のみ** 呼ぶ（経路3 D6 では既に dropped なので呼ばない）。Pass 2 は `markForDrop` に常に `residual.Currency`（非空）を渡す。`weightCurrencyFallback` は Pass 2 では使わない。
- `markForDrop("")` はどの経路からも呼ばれない（Pass 1 は `weightCurrencyFallback(p)` で `p.Amount != nil` 保証、Pass 2 は `residual.Currency` で非空）→ `dropped[""]` は常に false → `currency == ""` の未解決 unknown は Step 5 で除外されず `txn.Postings` に残る（現状の「Pass 2 失敗時は posting を残す」挙動と一致）。

##### Step 4 完了時点の externally observable な挙動（ロック対象）

Step 4 は drop を適用しない。追加変更は全て `pr.bookedDesc` / `pr.unknownDesc[k].currency` / `pr.dropped` への書き込みで、Step 5 まで誰も読まない。`finalize` は `bookedDesc` を走るが Step 4 の append は `finalize` 実行後なので挙動不変。よって Refactor-2 後の状態に対し **behavior-preserving**。`TestReducerWalk_AutoPostingResidualUsesBookedReductions` / `TestReducerWalk_AutoPostingZeroResidual` / `TestReducerWalk_MultipleAutoPostings` / `TestReducerWalk_Step3_FailedGroupDroppedFlagSet` 等は **テストファイル無改変で合格**。

##### Step 5 への cross-step coupling 固定点

1. `bookedDesc` は Pass 1 + Pass 2 の全生存候補 `BookedPosting` を `{currency, postingAt}` 付きでカバー。`booked` と `bookedDesc` は parallel（同一長・同一順）— Pass 2 の append も 1:1・同順を守る。
2. `unknownDesc` 各エントリは `currency == ""`（未解決＝無条件生存）/ `currency != "" && !dropped[currency]`（生存 resolved unknown — `bookedDesc` にも同 postingAt）/ `currency != "" && dropped[currency]`（drop 対象 — D6 / 経路4 bookOne 失敗、`bookedDesc` に対応エントリ無し）のいずれか。
3. `txn.Postings` rebuild は `bookedDesc` ∪ `unknownDesc` の postingAt 集合のうち drop されないものを入力順で集める。同一 postingAt が両 descriptor に出る（生存 resolved unknown）ケースを許容。
4. `finalize` の呼び出し位置を Pass 2 完了後へ動かす／責務分割する判断は Step 5 が行う。Step 4 は現状位置を維持。

##### Suggested Internals

- `len(unknowns) == 1` 経路では `unknownDesc[0]` が唯一の unknown（`finalize` が `unknowns` を `unknownDesc` と同順で構築するため）。定数 `0` を使い根拠コメントを 1 行残す案を推奨。
- `pr.dropped` は nil でも `pr.dropped[residual.Currency]` は nil-safe（false を返す）。明示 nil チェック不要。
- `booked` への append と `pr.bookedDesc` への append を必ず 1:1・同順で。崩れると Step 5 の parallel 前提が壊れる。代替: Pass 2 が `pr.booked` 自体に append し直して `pr.booked`/`pr.bookedDesc` を構造的に parallel に保つ案も可（generator 判断）。
- D6 経路と経路4 を共通関数に括り出すかは generator 判断（D6 で posting 本体を touch しない方針は維持）。
- 新規テストは最小限。既存テスト全件再合格を主検証とし、drop の本格検証は Step 5 に委ねる。
